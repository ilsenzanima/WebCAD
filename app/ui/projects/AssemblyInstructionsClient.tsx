        ...
        if (isPezzoUnico) {
          ...
          return [
            ...
            },
            {
              num: 2,
              title: "📐 Costruzione del Tratto d'Ingresso (Tratto 1)",
              desc: `Prepara a terra le lastre del Tratto 1 (ingresso). Taglia i fianchi sinistro e destro, la schiena e il fronte a lunghezza intera pari a ${(L_in + heightCm/2 + thicknessCm).toFixed(1)} cm, tranne il fianco destro (interno corto) che viene tagliato a ${(L_in - heightCm/2 - thicknessCm).toFixed(1)} cm per lasciare aperta la luce. *IMPORTANTE: Incollare e avvitare lungo tutti i bordi con collante tagliafuoco e viti.*`,
              materials: [
                `2x Fondo/Coperchio (Schiene/Fronti) Tratto 1: ${(widthCm + 2 * thicknessCm).toFixed(1)} x ${(L_in + heightCm/2 + thicknessCm).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                `1x Fianco Esterno Tratto 1 (Lungo): ${(heightCm + 2 * thicknessCm).toFixed(1)} x ${(L_in + heightCm/2 + thicknessCm).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                `1x Fianco Interno Tratto 1 (Corto): ${(heightCm + 2 * thicknessCm).toFixed(1)} x ${(L_in - heightCm/2 - thicknessCm).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                "Viti e collante tagliafuoco"
              ],
            },
            {
            ...
            },
            {
              num: 4,
              title: "🧱 Costruzione del Tratto d'Uscita (Tratto 2)",
              desc: `Prepara le lastre per il Tratto 2 (uscita). Taglia i fianchi e il fronte a ${(L_out - heightCm/2).toFixed(1)} cm per andare in battuta sul Tratto 1. Taglia la schiena (esterno lungo) a ${(L_out + heightCm/2 + thicknessCm).toFixed(1)} cm in modo da sormontare la testa del Tratto 1 e chiudere l'angolo esterno della curva. *IMPORTANTE: Applicare il collante tagliafuoco e le viti.*`,
              materials: [
                `1x Schiena Tratto 2 (Lunga): ${(widthCm + 2 * thicknessCm).toFixed(1)} x ${(L_out + heightCm/2 + thicknessCm).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                `1x Fronte Tratto 2 (Corto): ${(widthCm + 2 * thicknessCm).toFixed(1)} x ${(L_out - heightCm/2).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                `1x Fianco Esterno Tratto 2 (Lungo): ${(heightCm + 2 * thicknessCm).toFixed(1)} x ${(L_out + heightCm/2 + thicknessCm).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                `1x Fianco Interno Tratto 2 (Corto): ${(heightCm + 2 * thicknessCm).toFixed(1)} x ${(L_out - heightCm/2).toFixed(1)} cm (Spessore: ${thicknessMm} mm)`,
                "Viti e collante tagliafuoco",
              ],
            },
            {
