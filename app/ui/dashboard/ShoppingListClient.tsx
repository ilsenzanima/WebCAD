...
import { DeleteIcon, EditIcon, CheckIcon } from "./icons";
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = editingProductId
        ? await updateShoppingProduct(editingProductId, payload)
        : await createShoppingProduct(payload);
      if (!res.success || !res.data) { alert(res.error); return; }
      const product = res.data as ShoppingProduct;
      setProducts((prev) => (editingProductId ? prev.map((p) => (p.id === editingProductId ? product : p)) : [...prev, product]));
      resetProductForm();
    });
  };

  const handleEditProduct = (p: ShoppingProduct) => {
    setEditingProductId(p.id);
    setProdName(p.name);
    setProdCategory(p.category || "");
    setProdStore(p.default_store || "");
    setProdAisle(p.aisle || "");
    setProdUnit(p.default_unit || "");
    setProdShelfLife(p.shelf_life_days?.toString() || "");
  };

  const handleDeleteProduct = (id: string) => {
    if (!confirm("Rimuovere questo prodotto dalla vetrina?")) return;
    startTransition(async () => {
      ...
      <div className="animate-fade-in flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 p-1 bg-zinc-950/80 border border-white/10 rounded-2xl w-fit">
          <button type="button" onClick={() => setActiveTab("list")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "list" ? "bg-white/10 text-white shadow-lg" : "text-zinc-400 hover:text-white"}`}>
            📝 Lista
          </button>
          <button type="button" onClick={() => setActiveTab("catalog")}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === "catalog" ? "bg-white/10 text-white shadow-lg" : "text-zinc-400 hover:text-white"}`}>
            🗂️ Vetrina prodotti
          </button>
        </div>
      </div>
      ...
      {activeTab === "catalog" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
          <div className="rounded-2xl p-6 border shadow-2xl backdrop-blur-xl h-fit"
            ...
            <h2 className="text-base font-extrabold text-white mb-4">{editingProductId ? "Modifica prodotto" : "Nuovo prodotto"}</h2>
            <form onSubmit={handleProductSubmit} className="space-y-3">
              ...
              <p className="text-[9px] text-zinc-500">"Dura gg" = giorni tipici prima della scadenza da quando viene acquistato (solo per i freschi).</p>
              <div className="flex gap-2 pt-1">
                {editingProductId && (
                  <button type="button" onClick={resetProductForm} className="flex-1 py-3 rounded-xl text-xs font-bold text-slate-300 bg-zinc-800 hover:bg-zinc-700 transition-all">
                    Annulla
                  </button>
                )}
                <button type="submit" disabled={isPending} className="flex-1 py-3 rounded-xl text-xs font-extrabold text-white bg-indigo-600 hover:bg-indigo-500 transition-all">
                  {editingProductId ? "Salva" : "Crea prodotto"}
                </button>
              </div>
            </form>
          </div>
              ...
              {Object.entries(productsByCategory).map(([category, items]) => (
                <div key={category}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {items.map((p) => (
                      <div key={p.id} className="p-3 rounded-xl border border-zinc-800/80 bg-zinc-950/40 flex items-center justify-between group">
                        <div>
                          <div className="text-xs font-bold text-white">{p.name}</div>
                          <div className="text-[10px] text-zinc-500">
                            {[p.default_store, p.aisle, p.shelf_life_days ? `dura ${p.shelf_life_days}g` : null].filter(Boolean).join(" · ") || "—"}
                          </div>
                        </div>
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEditProduct(p)} className="p-1 rounded text-slate-400 hover:text-white transition-all">
                            <EditIcon size={12} />
                          </button>
                          <button onClick={() => handleDeleteProduct(p.id)} className="p-1 rounded text-slate-400 hover:text-rose-400 transition-all">
                            <DeleteIcon size={12} />
                          </button>
                        </div>
                      </div>
